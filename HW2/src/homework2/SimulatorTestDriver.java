...
public class SimulatorTestDriver {
	 ...
	 * @effects Creates a new Channel named by the String channelName,
	 * 			with a limit, and add it to the simulator named simName.
	 */
	 ...
	 *           && participantName has not been used in a previous
	 *           addParticipant(), addChannel() call on this object
	 *			 amount > 0
	 *			 product must be a single word, without special
	 *			 characters/number and also in lowercase
	 * @modifies simulator named simName
	 * @effects Creates a new Participant named by the String participantName
	 * 			and add it to the simulator named simName.
	 */
	public void addParticipant(String simName, String participantName,
							   String product, int amount) throws
			NodeLabelAlreadyExistsException
	{
		Simulator<String, Transaction> simulator = simulators.get(simName);
	 ...
	 */
	public void addEdge(String simName, String parentName, String childName,
	 ...
	 */
	public void sendTransaction(String simName, String channelName,
			...
			NodeLabelDoesNotExistException
	{
		...
		List<Transaction> transactionValues =
				simulator.getPipeByLabel(channelName).getPipeObjectsBuffer();
		String[] str = transactionValues.stream().map(Transaction::toString).
							toArray(String[]::new);
	 ...
	 * @return The sum of all Transaction amount of stored products that one has
	 * 		   in his storage buffer.
	 */
	public double getParticipantStorageAmount(String simName,
											  String participantName) throws
			NodeLabelDoesNotExistException
	{
		Simulator<String, Transaction> simulator = simulators.get(simName);
		Participant participant =
				(Participant)simulator.getFilterByLabel(participantName);
	 ...
	 */
	public void printAllEdges(String simName) throws
			NodeLabelDoesNotExistException
	{
		Simulator<String, Transaction> simulator = simulators.get(simName);
